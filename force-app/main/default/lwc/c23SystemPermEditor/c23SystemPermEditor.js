...
import applySystemPermissionsJsonEnvelope from '@salesforce/apex/c23_PermissionStudioSystemPermService.applySystemPermissionsJsonEnvelope';
import getSystemPermissionMatrix from '@salesforce/apex/c23_PermissionStudioSystemPermService.getSystemPermissionMatrix';
import c23_PS_Error_InvalidPermissionSet from '@salesforce/label/c.c23_PS_Error_InvalidPermissionSet';
import c23_PS_System_ApplySuccess from '@salesforce/label/c.c23_PS_System_ApplySuccess';
import c23_PS_System_Column_ApiName from '@salesforce/label/c.c23_PS_System_Column_ApiName';
import c23_PS_System_Column_Enabled from '@salesforce/label/c.c23_PS_System_Column_Enabled';
import c23_PS_System_Column_Name from '@salesforce/label/c.c23_PS_System_Column_Name';
import c23_PS_System_Preset_DisableAll from '@salesforce/label/c.c23_PS_System_Preset_DisableAll';
import c23_PS_System_Preset_EnableAll from '@salesforce/label/c.c23_PS_System_Preset_EnableAll';
import c23_PS_System_Subtitle from '@salesforce/label/c.c23_PS_System_Subtitle';
...
export default class C23SystemPermEditor extends LightningElement {
    ...
    labels = {
        title: c23_PS_System_Title,
        subtitle: c23_PS_System_Subtitle,
        invalidPermissionSet: c23_PS_Error_InvalidPermissionSet,
        columnName: c23_PS_System_Column_Name,
        columnApiName: c23_PS_System_Column_ApiName,
        columnEnabled: c23_PS_System_Column_Enabled,
        presetEnableAll: c23_PS_System_Preset_EnableAll,
        presetDisableAll: c23_PS_System_Preset_DisableAll,
        applySuccess: c23_PS_System_ApplySuccess,
    ...
    get tableWrapClass() {
        return `c23-system__table-wrap ${this.densityClass}`;
    }
    ...
    async loadMatrix() {
        ...
        this.errorMessage = '';
        try {
            const data = await getSystemPermissionMatrix({ parentPermissionSetId: this._permissionSetId });
    ...
    applyEnabledToVisibleRows(enabledValue) {
        ...
        const visibleKeys = new Set(this.visibleRows.map((row) => row.apiName));
        this.rows = this.rows.map((row) => {
            if (!visibleKeys.has(row.apiName)) {
                return row;
            }
    ...
    async handleApply() {
        ...
        try {
            ...
            const response = await applySystemPermissionsJsonEnvelope({
